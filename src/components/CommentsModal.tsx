...
export default function CommentsModal({ post, isOpen, onClose, onCommentCountUpdated, onPostDeleted, onLikeUpdated }: CommentsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-2 sm:p-4 animate-fade-in" id="comments-overlay">
      <div className="bg-zinc-950 border border-zinc-900 rounded-2xl w-full max-w-lg h-[92vh] sm:h-[85vh] flex flex-col overflow-hidden shadow-2xl relative transition-all">
        {showConfirmDelete && (
          <div className="absolute inset-0 bg-black/95 z-50 flex flex-col items-center justify-center p-6 text-center space-y-4 animate-fade-in">
        ...
        {/* HEADER BLOCK - Identical to Home feed card */}
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-zinc-900 bg-zinc-950">
          ...
          <div className="flex items-center gap-2">
            {(post.userId === user?.id || user?.role === 'admin') && (
              <button
                ...
                className="text-zinc-500 hover:text-rose-500 p-2 rounded-lg hover:bg-rose-950/10 transition-colors cursor-pointer"
                title="Delete Post"
              >
        ...
        {/* SCROLLABLE INTERMEDIATE FRAME */}
        <div className="flex-1 overflow-y-auto no-scrollbar">
          {/* MEDIA BOX - Identical visual size to feed posts */}
          {hasMedia && (
            <div className="relative w-full aspect-square bg-zinc-900 overflow-hidden flex items-center justify-center border-b border-zinc-900">
              {isVideo ? (
                <video
                    ...
                    {post.mediaUrls.map((_, dIdx) => (
                      <div
                        ...
                        className={`h-1.5 rounded-full transition-all ${
                          dIdx === activeIndex ? 'w-4 bg-indigo-500' : 'w-1.5 bg-white/40'
                        }`}
                      />
                    ))}
                  </div>
          ...
          {/* ACTION BUTTONS TOOLBAR - Unified Home UI style */}
          <div className="p-4 pb-2 flex items-center justify-between border-b border-zinc-900/50 bg-black/10">
            <div className="flex items-center gap-4">
              ...
              >
                <Heart className={`h-5 w-5 transition-transform duration-200 active:scale-150 ${user && likes.includes(user.id) ? 'fill-rose-500 text-rose-500 scale-105' : 'text-zinc-400 hover:text-white'}`} />
                <span className={user && likes.includes(user.id) ? 'text-rose-450 font-bold' : 'text-zinc-450'}>
                  {likes.length}
                </span>
              </button>
          ...
          <div className="p-4 space-y-4">
            <div className="flex items-start gap-3">
              <img 
                ...
                className="h-8 w-8 rounded-full object-cover border border-zinc-850 cursor-pointer mt-0.5" 
              />
          ...
          {/* COMMENTS DISPLAY DISCUSSION PANEL */}
          <div className="p-4 pt-1 border-t border-zinc-900/60 bg-zinc-950">
            <h4 className="text-[10px] uppercase font-mono tracking-widest text-zinc-500 mb-4 pb-1.5 border-b border-zinc-900">
              Community Comments ({comments.length})
            </h4>

            {comments.length === 0 ? (
              <div className="py-8 text-center text-zinc-500 text-xs flex flex-col items-center justify-center">
                <span className="text-xl mb-1">💬</span>
                <span>Silent timeline here. Express your ideas first!</span>
              </div>
            ) : (
              <div className="space-y-4">
                {comments.map(com => (
                  <div key={com.id} className="flex gap-3 group animate-slide-in">
                    ...
                    />
                    <div className="flex-1 bg-zinc-900/30 p-2.5 rounded-xl border border-zinc-900/60">
                      ...
                      <p className="text-xs text-zinc-300 mt-1 pr-4 leading-relaxed">{com.content}</p>
                    </div>

                    {(com.userId === user?.id || post.userId === user?.id || user?.role === 'admin') && (
                      <button
                        onClick={() => handleDeleteComment(com.id)}
                        className="opacity-0 group-hover:opacity-100 text-zinc-500 hover:text-rose-450 p-1.5 rounded transition-all cursor-pointer self-center"
        ...
        {/* BOTTOM STICKY REACTION INPUT FORM */}
        <form onSubmit={handlePostComment} className="border-t border-zinc-900 p-4 bg-zinc-950 flex gap-2">
          <input
            type="text"
            value={newComment}
            onChange={e => setNewComment(e.target.value)}
            placeholder="Chime into Pulse conversation..."
            className="flex-1 bg-zinc-900 border border-zinc-800 text-xs rounded-xl px-4 py-2.5 text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-indigo-500 transition-colors"
            disabled={loading}
          />
          <button
            type="submit"
